...
export function initializeCanvas(counties, usTopo, worldTopo, cases, date, options, updateLegend) {
  ...
  draw(
    world.coordinates,
    new THREE.LineBasicMaterial({color: '#BBBBBB', side: THREE.DoubleSide})
  );
  draw(
    us.coordinates,
    new THREE.LineBasicMaterial({color: '#BBBBBB', side: THREE.DoubleSide})
  ...
  counties.data.forEach((county, index) => {
    ...
    //   )
    // }
    // cylinderGeometry.position.x = county[countyIndex.x]
    // cylinderGeometry.position.y = county[countyIndex.y]
    // cylinderGeometry.position.z = county[countyIndex.z]
    // cylinderGeometry.scale.set(1, cylinderHeight, 1)
    // cylinderGeometry.quaternion.setFromUnitVectors(
    ...
    cylinders[index] = cylinderGeometry
  })
  const mergedGeometry = BufferGeometryUtils.mergeBufferGeometries(cylinders, false);
  const cylinder = new THREE.Mesh(mergedGeometry, originalCylinderMaterial)
  scene.add(cylinder)
